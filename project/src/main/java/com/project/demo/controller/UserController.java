...
public class UserController extends BaseController<User, UserService> {
    /**
     * Redis 不可用时的登录态兜底（内存 token 存储）。
     * 说明：当前场景 Redis(6379) 未启动会导致 login 写 token 抛异常 500。
     * 兜底后仍可正常登录与鉴权；若后续 Redis 启动，则优先走 Redis。
     */
    private static final ConcurrentHashMap<String, AccessToken> LOCAL_TOKEN_CACHE = new ConcurrentHashMap<>();
    private static final ConcurrentHashMap<String, Long> LOCAL_TOKEN_EXPIRE_AT = new ConcurrentHashMap<>();
    ...
    public Map<String, Object> login(@RequestBody Map<String, String> data, HttpServletRequest httpServletRequest) {
        ...
        if (!StringUtils.isEmpty(face) && face.equals("face")){
            ...
            if (byUsername.getPassword().equals(md5password)) {
                ...
                // 优先写入 Redis；Redis 不可用时降级到内存兜底
                try {
                    redisTemplate.opsForValue().set(accessToken.getToken(), accessToken, duration);
                } catch (Exception redisEx) {
                    LOCAL_TOKEN_CACHE.put(accessToken.getToken(), accessToken);
                    LOCAL_TOKEN_EXPIRE_AT.put(accessToken.getToken(),
                            System.currentTimeMillis() + duration.toMillis());
                    log.warn("[Redis token store failed, fallback to local cache] token={}, err={}",
                            accessToken.getToken(), redisEx.getMessage());
                }
    ...
    public Map<String, Object> quit(HttpServletRequest request) {
        ...
        try{
            ...
            // Redis 不可用时也要清理本地兜底缓存
            LOCAL_TOKEN_CACHE.remove(token);
            LOCAL_TOKEN_EXPIRE_AT.remove(token);
            log.warn("[Redis token delete failed, cleared local token] token={}, err={}", token, e.getMessage());
        }
    ...
    public Integer tokenGetUserId(String token) {
        ...
        try {
            ...
            // Redis 不可用：走内存兜底
            Long expireAt = LOCAL_TOKEN_EXPIRE_AT.get(token);
            if(expireAt == null || System.currentTimeMillis() > expireAt){
                LOCAL_TOKEN_CACHE.remove(token);
                LOCAL_TOKEN_EXPIRE_AT.remove(token);
                return 0;
            }
            AccessToken byToken = LOCAL_TOKEN_CACHE.get(token);
            return (byToken == null || byToken.getUser_id() == null) ? 0 : byToken.getUser_id();
        }
    }
